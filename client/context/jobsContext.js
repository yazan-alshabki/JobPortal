...
export const JobsContextProvider = ({ children }) => {
  ...
  const searchJobs = async (tags, location, title, minSalary, maxSalary, skills) => {
    setLoading(true);
    try {
      ...
      if (minSalary !== undefined) query.append("minSalary", minSalary);
      if (maxSalary !== undefined) query.append("maxSalary", maxSalary);
      if (skills && skills.length > 0) query.append("skills", skills.join(","));  // <-- important
  ...
  const applyToJob = async (jobId) => {
    ...
    try {
      ...
      toast.success("Applied to job successfully. Please wait for an email.", { duration: 4000 });
      getJobs();
    } catch (error) {
